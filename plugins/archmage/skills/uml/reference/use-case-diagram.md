...
Building this diagram in Enterprise Architect — the diagram/element/connector `type` strings (incl. the `Boundary` system boundary) and the COM display fixes (headless «include»/«extend» arrows) — is a **tool** concern. See the **`ea-modeling`** skill (`reference/diagram-type-playbooks.md` for the build quirks, `reference/notation-to-ea-mapping.md` for the type mapping) and `${CLAUDE_PLUGIN_ROOT}/shared/reference/ea-type-cheatsheet.md` for the canonical type strings.
